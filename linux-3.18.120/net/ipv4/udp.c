		...
		size_t len)
{
	...
	if (msg->msg_name) {
		...
		/* 没有指定目的地址，并且以前没有调用过connect */ 
		//也就是说udp发送send的时候可以不指定目的地址，但必须在发送前connect。就和TCP一样
...
do_confirm:
	/* 应用层知道网关可达，因此直接对路由缓存项进行确认 */
	dst_confirm(&rt->dst);
	/* MSG_PROBE标志表示仅仅用来发现路径，并不正直发送数据。如果没有指定这个标志，则发送数据 */
